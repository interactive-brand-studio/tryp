...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <form method="POST" action="{{ route('admin.hotels.store') }}" enctype="multipart/form-data" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Price -->
                <div>
                    <label for="price" class="block text-sm font-medium text-gray-700 mb-1">Starting Price (USD)</label>
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <span class="text-gray-500 sm:text-sm">$</span>
                        </div>
                        <input type="number" name="price" id="price" step="0.01" min="0" value="{{ old('price') }}" class="focus:ring-blue-500 focus:border-blue-500 block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-md" placeholder="0.00">
                    </div>
                    @error('price')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="seo_title" id="seo_title" value="{{ old('seo_title') }}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" placeholder="SEO friendly title">
                    @error('seo_title')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="seo_description" id="seo_description" rows="3" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" placeholder="Brief description for search engines (max 500 characters)">{{ old('seo_description') }}</textarea>
                ...
                <!-- SEO Image -->
                <div class="col-span-2">
                    <label for="seo_image" class="block text-sm font-medium text-gray-700 mb-1">SEO Image</label>
                    <div class="seo-image-upload p-4 border border-gray-200 rounded-lg">
                        <input type="file" name="seo_image" id="seo_image" accept="image/*" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                ...
                <div class="col-span-2">
                    ...
                    <button type="button" id="add-amenity" class="mt-2 px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600">Add Amenity</button>
                    @error('amenities')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Photos -->
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Hotel Photos</label>
                    ...
                    <button type="button" id="add-photo" class="mt-2 px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600">Add Photo</button>
                    <p class="text-sm text-gray-500 mt-1">Maximum 10 photos. Accepted formats: JPEG, PNG, JPG, GIF. Max size: 2MB per photo.</p>
                    @error('photos')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    @error('photos.*')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('add-photo').addEventListener('click', function() {
        ...
        const photoCount = container.children.length;
